...
from utilities import get_gd_optimizer, get_gd_directory, get_loss_and_acc, compute_losses, \
    ...
    compute_grad_h_grad, second_order_approx, \
    get_preconditioned_hessian_eigenvalues, compute_neg_elbo, get_trace_hessian_cube, get_trace, \
    evaluate_rhs_descent,calculate_hcrit_adjusted, calculate_hcrit_projected, second_order_loss_expansion
...
def main(dataset: str = "cifar10-10k", arch_id: str = "resnet32", loss: str = "ce", opt: str = "gd", lr: float = 1e-2, max_steps: int = 10000, neigs: int = 0,
    ...
    for step in range(0, max_steps):
        ...
        if eig_freq != -1 and step % eig_freq == 0 and step !=0:
           ...
           # if opt=="ivon":
            ...
            #     print(("neg_elbo:", neg_elbo[step//eig_freq]))
                ...
                # cubed_eigs = get_trace_hessian_cube(network, loss_fn, train_dataset, physical_batch_size=physical_batch_size, n_iters=50, device=device)
                # trace = get_trace(network, loss_fn, train_dataset, physical_batch_size=physical_batch_size, n_iters=50, device=device)
                # descent_rhs = evaluate_rhs_descent(network, loss_fn, train_dataset,lr,post_samples,h0,ess, physical_batch_size=physical_batch_size, n_iters=50, device=device)
                # print("descent_rhs is",descent_rhs)
                # print("trace is",trace)
                #print("preconditioned eigenvalues",pre_eigs[step // eig_freq, :])
                #Calculate H_crit
                #Hcrit = calculate_hcrit_adjusted(network, loss_fn, train_dataset,lr,post_samples,h0,ess, physical_batch_size=physical_batch_size, device=device)
                #hcrits[step // eig_freq, :] = calculate_hcrit_projected(network, loss_fn, train_dataset,lr,post_samples,h0,ess, physical_batch_size=physical_batch_size, device=device,neigs=neigs)
                #second_order_loss[step // eig_freq] = second_order_loss_expansion(network, loss_fn, train_dataset,lr,post_samples,h0,ess, physical_batch_size=physical_batch_size, device=device)
               
                #print("H_crit value_proj:", Hcrit_proj)
            # if opt=="ivon":
        ...
        if save_freq != -1 and step % save_freq == 0:
            save_files(directory, [("eigs", eigs[:step // eig_freq]), ("iterates", iterates[:step // iterate_freq]), ("hcrits", hcrits[:step // eig_freq]),
    ...
    save_files_final(directory,
                     [("eigs", eigs[:(step + 1) // eig_freq]), ("iterates", iterates[:(step + 1) // iterate_freq]), ("hcrits", hcrits[:(step + 1) // eig_freq]),
                      ("pre-eigs", pre_eigs[:step // eig_freq]),
                      ("neg-elbo", neg_elbo[:step // eig_freq]),
