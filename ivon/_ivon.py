...
class IVON(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        #     raise ValueError("Invalid beta2 parametxer: {}".format(beta2))
    ...
    def _restore_param_average(
        ...
        if train:  # collect grad sample for training
            ...
            #print("avg_grad in restore func is", self.state["avg_grad"])
    ...
    def step(self, closure: ClosureType = None) -> Optional[Tensor]:
        #print("mc samples are",self.mc_samples)
        if closure is None:
            #print("closure is none")
            loss = None
        else:
            losses = []
    ...
    @torch.no_grad()
    def get_avg_grad(self) -> Optional[Tensor]:
        """
        Returns the current average gradient.
        """
        return self.state.get('avg_grad', None)    
    ...
    def _update(self):
        ...
        for group in self.param_groups:
            ...
            #print("avg_grad in update function is ",self.state["avg_grad"])
